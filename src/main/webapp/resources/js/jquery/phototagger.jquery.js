...
(function( window, $ ){
	...
	function PhotoTagger( container, settings ){
		...
		if (this.settings.activateOnHover == true) {
		...
		this.container.mousedown(
			function( event ){
			    if (event.which === 1 && !self.isUserCreatingTag()) {
	...
	PhotoTagger.prototype = {
		...
		drawMode: function() {
		    ...
		    this.enableTagCreation();
		    this.disableTagDeletion();
		},
		...
		editMode: function() {
		    ...
		    this.disableTagCreation();
		    this.enableTagDeletion();
		},
		...
		addTag: function( id, x, y, width, height, message ){
			...
			tag.bind(
				...
				function(){
				    ...
				    if (self.mode === "view") {
				    	self.viewTag(tag.data( "id" ));
				    }
				}			
			);
			
			//bind the click to drag & resize
			tag.bind(
			    "click.photoTagger", 
			    function() {
			        if (self.mode === "view" || self.mode === "draw") {
			            return;
			        }
			        
			        if (!tag.hasClass("ui-draggable") && !tag.hasClass("ui-resizable")) {
			            self.tags.resizable("destroy").draggable( "destroy");
			...
			if (this.settings.additionalTagBehavior) {
			    this.settings.additionalTagBehavior.call(tag);
			}
...
//			this.deleteTagRecord( tag.data( "id" ) );
		...
		emphasizeTag: function( tag ){
			...
			this.message
				.css({
					...
					//top: ((tagPosition.top + tag.outerHeight() - this.message.outerHeight()) + "px")
				...
				function( response ){
					...
					$.each(
						...
						function( index, tagData ){
							...
							self.addTag(
								tagData.fieldId,
								tagData.tagInfo.leftTagPosition,
								tagData.tagInfo.topTagPosition,
								tagData.tagInfo.tagWidth,
								tagData.tagInfo.tagHeight,
								tagData.fieldName
							);
		...
		reloadTags: function() {
		    this.tags.resizable("destroy").draggable( "destroy");
		    this.tags.remove();
		    this.tags = $([]);
		...
		setupPendingTagCreation: function( clickX, clickY ){
			...
			if (self.settings.beforeDrawPendingTag) {
			    self.settings.beforeDrawPendingTag(self);
			}
		...
		viewTag: function(id) {
		    return this.settings.viewTag(id);
		},
		...
		clearPendingTag: function() {
		    if (this.settings.beforeRemovePendingTag) {
		        this.settings.beforeRemovePendingTag(this);
		    }
	...
	var applyPhotoTaggerMethod = function( collection, methodName , params){
	...
	$.fn.photoTagger = function(){
		...
		if (typeof( arguments[ 0 ] ) == "string"){
		    ...
		    if (arguments.length > 1) {
		        return(
                        applyPhotoTaggerMethod( this, arguments[ 0 ], arguments[1] )
                );
		    } else {
	...
	$.fn.photoTagger.defaultOptions = {
		...
		//This is the function to ask for user input
		showPrompt: function() {
		...
		//The function to add additional behavior to tag
		additionalTagBehavior: null,
		
		beforeDrawPendingTag: null,
		
		beforeRemovePendingTag: null,
		...
		css: {
			"a.tag": {
				"background-image": "url( '/fieldcatalog/resources/images/transparent.gif' )",
